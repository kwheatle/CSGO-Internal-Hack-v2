...
auto table_is_hooked(void* table, const char* module_table_dst) -> const bool {
	...
	const auto nt_headers = reinterpret_cast<PIMAGE_NT_HEADERS> (reinterpret_cast<uint8_t*>(module_base) + dos_header->e_lfanew);

	const auto module_end = reinterpret_cast<uintptr_t>(module_base) + nt_headers->OptionalHeader.SizeOfImage - sizeof(uintptr_t);

	const auto table_dst = *reinterpret_cast<uintptr_t*>(table);
	...
	return (table_dst < reinterpret_cast<uintptr_t>(module_base) || table_dst > module_end);
}
uintptr_t* vmt_hook::search_free_data_page(const char* module_name, const size_t vmt_size)
{
...
bool vmt_hook::setup(void* base, const char* moduleName) {
	if (base != nullptr)
		class_base = base;
...
size_t vmt_hook::estimate_vftbl_length(uintptr_t* vftbl_start) {
...
{
	new_vftb1[fn_index + 1] = reinterpret_cast<uintptr_t>(fn_pointer);
}

void vmt_hook::release() {
	try {
		if (old_vftbl != nullptr) {
			auto guard = protect_guard{ class_base, sizeof(uintptr_t), PAGE_READWRITE };
			*(uintptr_t**)class_base = old_vftbl;
			old_vftbl = nullptr;
		}
	}
	catch (...) {
