...
void Render::BeginScene() {
	...
	//CALL VISUALS
	//Render::Get().RenderText("DooDooWare TM", 10, 5, 18.f, Color(255,0,255,255), false, true);
	//Visuals::Get().DrawTest();
...
void Render::RenderCircle3D(Vector position, float points, float radius, Color color)
{
	...
	for (float a = 0; a < (M_PI * 2.0f); a += step)
	{
		...
		if (gInterface.debug->screen_position(start, start2d) || gInterface.debug->screen_position(end, end2d))
			return;
